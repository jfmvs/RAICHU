...
from TCN.attention import AttentionLayer
from keras.layers import Flatten, Reshape, Multiply
...
def short_term_embedding(args, node_list, idx, G):
    ...
    for i in range(len(ctdne_model.wv.key_to_index)):
        if ctdne_model.wv.index_to_key[i] in node_list:
            node_vec = ctdne_model.wv[ctdne_model.wv.index_to_key[i]]
            if node_vec is not None:
                node_embs[node_list.index(ctdne_model.wv.index_to_key[i])] = np.array(node_vec)
    return node_embs
...
class PIKACHU:
    ...
    def get_model_lkbck(self,upgrade):
        print('Lookback in')
        ...
        if upgrade:
            el1 = TCN(self.args.dimensions * 4)(mask)
            do_enc = Dropout(0.5)(el1)
            encoded = TCN(self.args.dimensions)(do_enc)
            rp = RepeatVector(self.args.lookback)(encoded)
            dl1 = TCN(self.args.dimensions)(rp)
            do_de = Dropout(0.5)(dl1)
            dl3 = TCN(self.args.dimensions*4)(do_de)
            #dl3 = AttentionLayer(attention_dim=self.args.dimensions*4)(dl3)
        else:
    ...
    def autoencoder_model(self, time_step, dim,upgrade):
        input = Input(shape=(time_step, dim))
        mask = Masking(mask_value=self.mask_val)(input)
        if upgrade:
            el1 = TCN(8,dilations = [1,2,4],kernel_size = 4, name='0')(mask)
            do_enc = Dropout(0.3)(el1)
            encoded = TCN(16,dilations = [1,2,4],kernel_size = 4,name='1')(do_enc)
            encoded = Flatten()(encoded)
            rp = RepeatVector(time_step)(encoded)
            dl1 = TCN(16,dilations = [1,2,4],kernel_size = 4,name='2')(rp)
            do_de = Dropout(0.3)(dl1)
            tcn_out = TCN(8,dilations = [1,2,4],kernel_size = 4,name='3')(do_de)
            dl3 = AttentionLayer(name="tcn_attention", attention_dim=512)(tcn_out)
            dl3 = RepeatVector(time_step)(dl3)
            dl3 = Multiply()([tcn_out,dl3])
        else:
            el1 = GRU(32, return_sequences=True)(mask)
            do_enc = Dropout(0.3)(el1)
            encoded = GRU(64, return_sequences=False)(do_enc)
            rp = RepeatVector(time_step)(encoded)
            dl1 = GRU(64, return_sequences=True)(rp)
            do_de = Dropout(0.3)(dl1)
            dl3 = GRU(32, return_sequences=True)(do_de)
        #do_enc = Dropout(0.3)(el1)
        # el2 = GRU(self.args.dimensions * 4, return_sequences=True, stateful=False)(do_enc)
        # do_enc1 = Dropout(0.5)(el2)
    ...
    def long_term_embedding(self, short_term_embs):
        ...
        print('TIME STEP TO EMBEDDING:',time_step)
        self.model, self.encoder = self.autoencoder_model(time_step, dim,True)
    ...
    def learn_embedding(self, save_file):
        ...
        dynamic_embs = np.transpose(dynamic_embs,(1,0,2))

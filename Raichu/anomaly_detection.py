...
def predict_prob(w, X):
    ...
    #return jax.nn.softmax(jnp.dot(X, w.T))
...
def calculate_edge_probability_gpu(w, node_emb, node_list, G):
...
class AnomalyDetection:
    ...
    def gradient_descent(self, w, X, Y, iterations, learning_rate):
        ...
        for i in range(iterations):
            ...
            costs.append(cost)
            print("Cost after iteration %i/%i:      %f" % (i+1, iterations, cost))
        params = {"w": w}
        return params, costs
    ...
    def get_train_edges(self, train_graphs, s = 10):
        data_x = []
        data_y = []
        with tqdm(total=len(train_graphs), mininterval=1) as pbar:
    ...
    def calculate_performance_metrics(self, edge_scores, result_file):
        ...
        return metrics.roc_auc_score(true_label, scores)

    def anomaly_detection(self, graphs, param_file):
        print("\n\nEstimating Edge Probability Distribution....")
        learning_rates = [0.0001] #0.1, 0.01, 0.001, 0.0001
        support_sets = [10,25]#2, 5,10, 15, 20
        # Lista de GPUs a serem utilizadas
        # Configura o TensorFlow para usar apenas as GPUs especificadas
        results = pd.DataFrame(columns=['Learning_rate','Sets','AUC'])
        for lr in learning_rates:
            for s in support_sets:
                ...
                print("\nNumber of GPU Available: ", total_cpu)
                ...
                tmp_auc = self.calculate_performance_metrics(edge_scores, result_file)
                tmp_row = {'Learning_rate':lr,'Sets':s,'AUC':tmp_auc}
                results = results.append(tmp_row,ignore_index=True)
